...
export class StoryHelper {
  ...
  ngOnInit() {
    ...
    this.characterService.getCharactersSplitTwins().subscribe(chars => {
      this.characters = chars.filter(c => ![0, 42, 71, 999].includes(c.id));
    });
  }
  ...
  generateCharacter() {
    ...
    // Create weighted array: tier 9 characters get 0.5 weight, others get 1.0 weight
    const weights = filteredCharacters.map(c => c.tier >= 9 ? 0.5 : 1.0);
